...
function calculateCoefficient(done, plan, coefficient) {
    // Проверяем, что все значения определены и не равны 0 для плана
    if (!done || !plan || coefficient === 'x' || coefficient === '×' || plan === 0) {
        return '×';
    }
    ...
    if (isNaN(doneNum) || isNaN(planNum) || isNaN(coeffNum)) {
        return '×';
    }

    // Расчет коэффициента с округлением до двух знаков
    const value = (doneNum / planNum) * coeffNum;
    return value.toFixed(2);
}

// Расчет всех коэффициентов
function calculateAllCoefficients() {
    let totalSum = 0;
    ...
    document.querySelectorAll('tr').forEach(row => {
        ...
        const done = input.value.trim(); // Получаем значение и удаляем пробелы
        ...
        const plan = planCell.textContent.trim();
        const coeff = coeffCell.textContent.trim();

        // Пропускаем строки с коэффициентом "×"
        if (coeff === '×' || coeff === 'x') {
            resultCell.textContent = '×';
            return;
        }

        // Для R-гр (пункт 11) особая обработка
        if (dataId === '11') {
            resultCell.textContent = '×'; // Всегда показываем ×
            // Но при этом считаем и добавляем в общую сумму
            const calculatedResult = calculateCoefficient(done, plan, coeff);
            if (calculatedResult !== '×') {
                totalSum += parseFloat(calculatedResult);
            }
        } else {
            // Для всех остальных пунктов обычная обработка
            const result = calculateCoefficient(done, plan, coeff);
    ...
    // Обновляем итоговый коэффициент
    const totalCoefficient = document.getElementById('totalCoefficient');
    totalCoefficient.textContent = totalSum.toFixed(2);
}
...
function setInputValue(id, value) {
    ...
    if (input) {
        input.value = Math.max(0, Math.round(value)); // Убеждаемся, что значение не отрицательное
    }
}